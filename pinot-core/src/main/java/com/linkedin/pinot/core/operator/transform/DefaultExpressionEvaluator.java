...
public class DefaultExpressionEvaluator implements TransformExpressionEvaluator {
   ...
   *
   * @param indexSegment Index segment to process
   * @param expressionTree Expression tree to evaluate
   */
  public DefaultExpressionEvaluator(@Nonnull IndexSegment indexSegment,
    ...
    if (function != null) {
      ...
      if (column != null) {
        if (isNumeric(column)) {
          ...
          throw new RuntimeException("Non numeric columns not supported in transforms yet");
        }
      } else {
  ...
  /**
   * Helper method to identify if a column is numeric.
   *
   * @param column Column to identify
   * @return True if data type is one of 'int'/'float'/'long'/'double', false otherwise.
   */
  private boolean isNumeric(String column) {
    FieldSpec.DataType dataType = _indexSegment.getDataSource(column).getDataSourceMetadata().getDataType();
    return (dataType == FieldSpec.DataType.INT || dataType == FieldSpec.DataType.FLOAT
        || dataType == FieldSpec.DataType.LONG || dataType == FieldSpec.DataType.DOUBLE);
  }
}
